# What this use for?
This is a tool that automatically builds the xidian SMPC demo environment. Please confirm that you have obtained the authorization from xidian to obtain the SMPC demo code.

# How to run?
1. `cd /path/to/xidiansMPC`
2. Update the config with appropriate parameters. Run `python -m aws.run-on-ec2` to start the AWS instances and run the xidiansmc command specified in the config. This command creates a `current.vms` file which consists of instance ids of the VMs created during this run. Subsequent runs of this command will reuse the same VMs.
...
# Configuration
1. In order to replace the docker image, update `image_path` in `aws-config.json` with the appropriate image.
